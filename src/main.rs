...
mod tests {
    ...
    // -- Custom rule test (verifies trait works) ---------------------------
    ...
    // -- Quiet mode filtering test -----------------------------------------

    #[test]
    fn quiet_mode_filters_warnings() {
        let dir = std::env::temp_dir().join("boxlint_test_quiet");
        let _ = fs::create_dir_all(&dir);
        let file = dir.join("sample.txt");
        fs::write(&file, "hello\n").unwrap();
        ...
        // With quiet=true, the lint function filters warnings internally.
